...
import {agreeFriends, delFriends, getAllFriendsList, refuseFriends} from "../../../apis/FriendMethods.tsx";
...
const Friends = () => {
    ...
    const agree = (key: number) => {
        const friend = staticReq.find(f => f.friendKey === key);
        if (!friend) return;
        ...
        agreeFriends(key, updateData).then((res) => {
            if(res.status === 200){
                initFriendsList()
                message.success('已添加')
            }
